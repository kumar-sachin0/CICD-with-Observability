pipeline {
    ...
    steps {
        withCredentials([usernamePassword(credentialsId: 'nexus-credentials', usernameVariable: 'NEXUS_USERNAME', passwordVariable: 'NEXUS_PASSWORD')]) {
            withMaven(globalMavenSettingsConfig: 'nexus-credentials', jdk: 'jdk17', maven: 'maven3', mavenSettingsConfig: 'nexus-credentials', traceability: true) {
                sh "mvn deploy -DrepositoryId=nexus -Dusername=$NEXUS_USERNAME -Dpassword=$NEXUS_PASSWORD -s $MAVEN_SETTINGS"
            }
        }
    }
}
        ...
        stage('Docker Image Scan') {
            steps {
                sh "trivy image --format table -o trivy-image-report.html sachin-kumar0/boardshack:latest "
            }
        }
